<template>
    <q-page class="q-pa-sm">
       <q-card flat>
         <q-table :grid="grid"  :filter="filter" flat bordered title="Category" :rows="categoryStore.subCategories"
          :columns="columns" virtual-scroll v-model:pagination="pagination">
           <template #top>
             <q-toolbar style="padding:0 !important;">
               ...
               <q-btn flat round dense icon="add" class="q-mr-xs" :to="localePath('/category/sub/add')" />
             ...
             <div class="col-xs-6 col-sm-4 col-md-3 col-lg-2 q-pa-xs">
               <q-card bordered :class="$q.dark.isActive ? 'hover:bg-gray-700':'hover:bg-gray-100'">
                 ...
                 <q-btn size="sm" flat icon="delete" color="red"
                   @click="deleteCategory(props.row._id, categoryStore.subCategories.indexOf(props.row))" />
               </q-card-actions>
